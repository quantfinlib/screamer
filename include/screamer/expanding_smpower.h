#ifndef SCREAMER_EXPANDINGSM_POWER_H
#define SCREAMER_EXPANDINGSM_POWER_H
...
namespace screamer {
    ...
    class ExpandingSMPower : public ScreamerBase {
    public:
        ExpandingSMPower(double phi, int tau)
            : phi(phi), tau(tau){
    ...
    private:
        ...
        double calculate_maximum_explosiveness() {
            ...
            for (size_t t0 = 0; t0 < (n > tau ? n - tau : 0); ++t0) {
                ...
                std::vector<double> log_t_sub(sub_size);

                std::iota(t_sub.begin(), t_sub.end(), 0);  // Generate t as the index vector
                std::transform(t_sub.begin(), t_sub.end(), log_t_sub.begin(),
                               [](double t) { return std::log(t+1); });

                try {
                    double explosiveness = compute_explosiveness(y_sub, log_t_sub);
...
#endif // SCREAMER_EXPANDINGSM_POWER_H
