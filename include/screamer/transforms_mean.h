#ifndef SCREAMER_TRANSFORMS_MEAN_H
#define SCREAMER_TRANSFORMS_MEAN_H
...
py::array_t<double> transform_1d_contiguous_mean(int window_size, py::array_t<double> input_array) {
    ...
    double factor = 1.0 / window_size; // multiplication is faster than division
...
py::array_t<double> transform_mean(int window_size, py::array_t<double> input_array) {
    double factor = 1.0 / window_size; // multiplication is faster than division
    ...
    if (buf_info.ndim == 1 && buf_info.strides[0] == sizeof(double)) {
        ...
        return transform_1d_contiguous_mean(window_size, input_array);
    }
