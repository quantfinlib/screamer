...
for i, delay in enumerate([10, 100, 1000, 10_000]):
    ...
    axs[0].loglog(Ns, time1s, color=f'C{i}', label=f'lag({delay})')
    axs[0].loglog(Ns, time2s, color=f'C{i}', ls='--', label=f'lagq({delay})')
    axs[1].semilogx(Ns, np.array(time2s) / np.array(time1s) - 1, color=f'C{i}', label=f'lagq({delay})')

axs[0].grid()
axs[0].set_xlabel('input length')
axs[0].set_ylabel('time (sec)')
axs[0].legend()
axs[0].set_title('Timing Lagq vs Lag')

axs[1].grid()
axs[1].set_xlabel('input length')
axs[1].set_ylabel('extra time pct Lagq')
axs[1].legend()
axs[1].set_title('Timing Lagq/Lag - 1')
...
save_path = os.path.join(script_dir, 'time_lag_vs_lagq.png')
